...
function createTranscodeStream(inputBuffer) {
    ...
    ffmpeg(inputStream)
        ...
        .on('error',    (err) => { console.error('❌ Transcode Error:', err.message); outputStream.destroy(err); })
        .on('end',      ()    => console.log('✅ Transcode Complete'))
...
router.post('/api/upload-track', upload.fields([{ name: 'audioFile', maxCount: 1 }, { name: 'artFile', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        ...
        let masterUrl;
        if (isLargeFile) {
            const ms = new PassThrough();
            ms.end(audioFile.buffer);
            masterUrl = await streamUploadToR2(ms, masterPath, audioFile.mimetype);
        } else {
        ...
        if (isLossless) {
            if (isLargeFile) {
                streamUrl = await streamUploadToR2(createTranscodeStream(audioFile.buffer), streamPath, 'audio/mpeg');
            } else {
                streamUrl = await uploadToStorage(await transcodeToBuffer(audioFile.buffer), streamPath, 'audio/mpeg');
            }
        ...
        const songData = {
            ...
            audioUrl: streamUrl, masterUrl, artUrl, originalFormat: fileExt, fileSize: audioFile.size, 
    ...
    try {
        ...
        for (let i = 0; i < audioFiles.length; i++) {
            ...
            try {
                ...
                let masterUrl;
                if (isLargeFile) {
                    const s = new PassThrough(); s.end(file.buffer);
                    masterUrl = await streamUploadToR2(s, masterPath, file.mimetype);
                ...
                if (isLossless) {
                    if (isLargeFile) {
                        streamUrl = await streamUploadToR2(createTranscodeStream(file.buffer), streamPath, 'audio/mpeg');
                    } else {
                        streamUrl = await uploadToStorage(await transcodeToBuffer(file.buffer), streamPath, 'audio/mpeg');
                    }
                } else { streamUrl = masterUrl; }
                ...
                const songData = {
                    ...
                    audioUrl: streamUrl, masterUrl, artUrl: albumArtUrl, originalFormat: fileExt, fileSize: file.size,
                    isLossless, isTranscoded: isLossless, bpm: features.bpm || 0, key: features.key || 'Unknown',
                    mode: features.mode || 'Unknown', energy: features.energy || 0, danceability: features.danceability || 0,
                    duration: features.duration || 0, city: locParts[0] || null, state: locParts[1] || null, country: locParts[2] || 'US',
                    stats: { plays: 0, likes: 0, downloads: 0 }, uploadedAt: admin.firestore.FieldValue.serverTimestamp()
...
//
// Uploads a single audio file as a streamable merch sample.
// Unlike a full track upload, this does NOT:
//   - require artwork
//   - create a songs/ document
//   - run audio analysis / BPM detection
//
// It DOES:
//   - transcode lossless files to 320k MP3 (same pipeline)
//   - upload to R2 at artists/{id}/merch-samples/
//   - return { success, streamUrl, title, duration }
//
// The returned streamUrl is saved as merch.sampleTrack.streamUrl.
// ==========================================
router.post('/api/upload-merch-sample',
    ...
    async (req, res) => {
        try {
            ...
            // All samples land as .mp3 in merch-samples/
            const r2Key = `artists/${artistId}/merch-samples/${timestamp}_${cleanTitle}.mp3`;

            let streamUrl;

            if (isLossless) {
                // Lossless → transcode to 320k MP3
                if (isLarge) {
                    streamUrl = await streamUploadToR2(
                        createTranscodeStream(req.file.buffer),
                        r2Key,
                        'audio/mpeg'
                    );
                } else {
                    const mp3Buffer = await transcodeToBuffer(req.file.buffer);
                    streamUrl = await uploadToStorage(mp3Buffer, r2Key, 'audio/mpeg');
                }
            } else {
                // Already MP3 / AAC — upload directly, no transcode needed
                if (isLarge) {
            ...
            // Get duration via a quick ffprobe (optional — gracefully skip on failure)
            let duration = null;
            try {
